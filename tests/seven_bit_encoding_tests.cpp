...
INSTANTIATE_TEST_SUITE_P(
    SevenBitEncoding, EncodedSizeTest,
    ::testing::Values(
        EncodedSizeTestCase{0, 1}, EncodedSizeTestCase{1, 1},
...
TEST(SevenBitEncoding, DecodeValue) {
  ...
  EXPECT_EQ(
      SevenBitEncoding::decodeValue(input.data(), input.size(), consumedBytes),
      127);
...
TEST(SevenBitEncoding, DecodeValueInvalidInput) {
  size_t consumedBytes;
  std::vector<uint8_t> input = {128 | 0x80, 128 | 0x80, 128 | 0x80, 128 | 0x80,
                                128 | 0x80};
  EXPECT_EQ(
      SevenBitEncoding::decodeValue(input.data(), input.size(), consumedBytes),
      0);
...
class GetEncodedBufferSizeTest
    : public ::testing::TestWithParam<EncodedSizeTestCase> {};
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(EncodedSizeTestCase{0, 1}, EncodedSizeTestCase{1, 2},
                      EncodedSizeTestCase{2, 3}, EncodedSizeTestCase{3, 4},
                      EncodedSizeTestCase{4, 5}, EncodedSizeTestCase{5, 6},
                      EncodedSizeTestCase{6, 7}, EncodedSizeTestCase{7, 8},
                      EncodedSizeTestCase{8, 10}, EncodedSizeTestCase{9, 11},
                      EncodedSizeTestCase{10, 12}, EncodedSizeTestCase{11, 13},
                      EncodedSizeTestCase{12, 14}, EncodedSizeTestCase{13, 15},
                      EncodedSizeTestCase{14, 16}, EncodedSizeTestCase{15, 18},
                      EncodedSizeTestCase{16, 19}, EncodedSizeTestCase{18, 21},
...
TEST_P(BufferEncodingTest, EncodeBuffer) {
  ...
  size_t encodedLength =
      SevenBitEncoding::encodeBuffer(tc.input.data(), tc.input.size(), encoded);
...
TEST_P(BufferEncodingTest, DecodeBuffer) {
  ...
  size_t decodedLength = SevenBitEncoding::decodeBuffer(
      tc.expectedEncoded.data(), tc.expectedEncoded.size(), decoded.data(),
      decoded.size());
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        BufferTestCase{{}, {}}, BufferTestCase{{0xFF}, {0xFF, 0x40}},
        BufferTestCase{{0x12, 0x34}, {0x89, 0x8D, 0x00}},
        BufferTestCase{{0xFF, 0xFF}, {0xFF, 0xFF, 0x60}},
        BufferTestCase{{0x00, 0x01, 0x80}, {0x80, 0x80, 0xB0, 0x00}},
        BufferTestCase{{0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06},
                       {0x80, 0x80, 0xA0, 0xA0, 0x98, 0x90, 0x8A, 0x06}},
        BufferTestCase{
            {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09},
            {0x80, 0x80, 0xA0, 0xA0, 0x98, 0x90, 0x8A, 0x86, 0x83, 0xC2, 0x81,
             0x10}},
        BufferTestCase{
        ...
        BufferTestCase{{0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06,
                       ...
                       {
                           0x80, 0x80, 0xA0, 0xA0, 0x98, 0x90, 0x8A, 0x86,
                           0x83, 0xC2, 0x81, 0x90, 0xD0, 0xAC, 0x98, 0x8D,
                           0x87, 0x83, 0xE2, 0x81, 0x88, 0xC8, 0x26,
                       }},
        BufferTestCase{
            {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A,
            ...
            {0x80, 0x80, 0xA0, 0xA0, 0x98, 0x90, 0x8A, 0x86, 0x83, 0xC2,
             0x81, 0x90, 0xD0, 0xAC, 0x98, 0x8D, 0x87, 0x83, 0xE2, 0x81,
             0x88, 0xC8, 0xA6, 0x94, 0x8A, 0xC5, 0xC2, 0xF1, 0xC0, 0xE4,
             0xB4, 0x9B, 0x8E, 0x87, 0xA3, 0xE1, 0x78}}));
...
TEST(SevenBitEncoding, FuzzEncodeDecode) {
  ...
  for (int i = 0; i < 1000; i++) {
    ...
    size_t decodedLen =
        SevenBitEncoding::decodeBuffer(encoded, encodedLen, decoded, len);
